...
export const AuditoriaView: React.FC<AuditoriaViewProps> = ({
  ...
  if (currentUser.role !== 'ADM') {
    return (
      <div className="p-8 sm:p-12 text-center bg-slate-900/60 border border-slate-800 rounded-2xl max-w-xl mx-auto space-y-4">
        ...
        <p className="text-xs text-slate-400 leading-relaxed">
          Seu papel atual é <strong>{currentUser.role}</strong>. Pelo modelo de Controle de Acesso Baseado em Papéis (RBAC) do SiSGO, somente administradores têm permissão para consultar os registros de auditoria da tabela <code className="text-indigo-400">LogSistema</code>.
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button
          onClick={onRefresh}
          disabled={loading}
          className="flex items-center gap-2 px-3.5 py-2 rounded-xl bg-slate-900 hover:bg-slate-800 text-slate-200 text-xs font-semibold border border-slate-800 transition-all cursor-pointer self-start sm:self-auto"
        >
      ...
      <div className="p-4 rounded-2xl bg-slate-900/80 border border-slate-800/80 flex flex-col sm:flex-row items-center justify-between gap-3">
        <div className="w-full sm:w-auto flex-1 max-w-md relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2 bg-slate-950 border border-slate-800 rounded-xl text-xs text-slate-200 placeholder-slate-500 focus:outline-none focus:ring-1 focus:ring-indigo-500/50"
          />
        </div>
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
          ...
          <select
            ...
            className="py-1.5 px-3 bg-slate-950 border border-slate-800 rounded-xl text-xs text-slate-300 focus:outline-none focus:ring-1 focus:ring-indigo-500/50 cursor-pointer"
          >
                ...
                filteredLogs.map((log) => (
                  <tr key={log.id} className="hover:bg-slate-800/30 transition-colors">
                    ...
                    <td className="py-3.5 px-4">
                      <div className="flex items-center gap-2">
                        <Terminal className="h-3.5 w-3.5 text-indigo-400 flex-shrink-0" />
                        <span className="font-sans font-medium text-slate-100 text-xs break-all">
                          {log.acao}
                        </span>
                      </div>
                    </td>
                    ...
                    <td className="py-3.5 px-4 font-sans">
                      {log.usuario ? (
                        <div>
                          <div className="text-slate-200 font-semibold">{log.usuario.nome}</div>
                          <div className="text-[10px] text-slate-500 font-mono">
                            @{log.usuario.login} ({log.usuario.role})
                          </div>
                        </div>
                      ) : (
                        <span className="text-slate-500">Sistema / Anônimo</span>
                      )}
                    </td>
                    ...
                    <td className="py-3.5 px-4 text-slate-500 text-[10px] max-w-[200px] truncate" title={log.userAgent || ''}>
