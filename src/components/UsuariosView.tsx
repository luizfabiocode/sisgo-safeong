  ...
  if (currentUser.role !== 'ADM') {
    return (
      <div className="p-8 text-center bg-slate-900/60 border border-slate-800 rounded-2xl max-w-xl mx-auto space-y-3">
        ...
        <h2 className="text-lg font-bold text-white">Acesso Exclusivo para Administrador</h2>
        <p className="text-xs text-slate-400">Somente usuários com papel ADM podem gerenciar os operadores.</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button
          onClick={onRefresh}
          disabled={loading}
          className="flex items-center gap-2 px-3.5 py-2 rounded-xl bg-slate-900 hover:bg-slate-800 text-slate-200 text-xs font-semibold border border-slate-800 transition-all cursor-pointer"
        >
                    ...
                    >
                      <span
                        className={`w-1.5 h-1.5 rounded-full ${
                          u.status === 'ATIVO' ? 'bg-emerald-400' : 'bg-rose-400'
                        }`}
                      />
                      {u.status}
                    </span>
